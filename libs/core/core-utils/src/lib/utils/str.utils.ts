export class StrUtils {
  /** ( 'aa/bb/cc', '/' ) => 'cc'  */
  static lastPartAfter( str: string, delimiter: string ): string {
    const p: string[] = str.split( delimiter )
    if ( p.length ) {
      return p[ p.length - 1 ]
    } else {
      return str
  ...
  /** ( 'aa/bb/cc', '/' ) => 'aa/bb'  */
  static stringBeforeLastString( str: string, delimiter: string ): string {
    const p: string[] = str.split( delimiter )
    if ( p.length ) {
      p.pop()
  ...
  static setFileExtFromAnotherFile(
    src: string,
    extFromFile: string,
    suffix = '',
  ): string {
    return (
      StrUtils.stringBeforeLastString( src, '.' ) +
      suffix +
      '.' +
      StrUtils.lastPartAfter( extFromFile, '.' )
    )
  }
  ...
  static stringBefore( str: string, tails: string[], caseInsensitive = false ) {
    ...
    for ( const tail of tails ) {
      if ( caseInsensitive ) {
        res = str.substring(
          0,
          str.toLowerCase().lastIndexOf( tail.toLowerCase() ),
        )
      } else {
  ...
  static repeatMap(
    ...
    const res = new Map()
    strArray.forEach( ( el ) => {
      if ( res.has( el ) ) res.set( el, res.get( el ) + 1 )
      else res.set( el, 1 )
    } )
    if ( !sortByCount ) {
      return res
  ...
  static getArrayOfUniqueValuesAndSortByRepeatCountDesc(
    strArray: string[],
  ): string[] {
    return StrUtils.arrayOfMapKeys( StrUtils.repeatMap( strArray, true ) )
  }

  static commonStringFromLeft(
    strArray: string[],
    ...
    for ( let i = 0; i < str1len; i++ ) {
      ...
      for ( let k = 1; k < size; k++ ) {
        if ( caseInsensitive ) {
          if (
            str1.substring( i, i + 1 ).toLowerCase() !==
            strArray[ k ].substring( i, i + 1 ).toLowerCase()
          ) {
            diff = true
            break
          }
