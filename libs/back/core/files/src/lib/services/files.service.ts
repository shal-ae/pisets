...
export class FilesService {
  ...
  async saveUploadedFile(
    ...
    const destinationFolder = path.join( this.config.filesDir, folder )
    ...
    const res: FileUploadResult = {
      ...
      filePath: FileUtils.toUrlPath( relativeFilename ),
      size: file.size,
      md5,
      originalName,
      renderedToPdf: false,
    ...
    if ( options.convertOfficeFileToPdf && OFFICE_CONVERT_EXT.includes( fileExt ) ) {
      extForThumbs.push( 'jpg' )
      const pdfFilePath =
        FileUtils.fileWithoutExt( destinationFilename ) + '.pdf'
      await this.convertOfficeService.convertOfficeFileToPdf( destinationFilename, pdfFilePath )
